export default function createFilterDropdown(
  e,
  labels,
  colors,
  currentFilters = [],
  ...
  listContainer.appendChild(
    Object.assign(document.createElement("div"), { id: "filter-labels" })
  );
  ...
  function updateSelection(checkbox, id) {
    ...
    selectAll.indeterminate =
      selected.size > 0 && selected.size < labels.length;
  ...
  labels.forEach((label, index) => {
    ...
    badge.className = `ms-2 badge rounded-pill text-bg-${
  ...
  selectAll.addEventListener("change", () => {
    const checkboxes = labelsContainer.querySelectorAll(
      "input[type='checkbox']"
    );
  ...
  labelsContainer.addEventListener("keydown", (e) => {
    const focusable = Array.from(
      labelsContainer.querySelectorAll("input[type='checkbox']")
    );
