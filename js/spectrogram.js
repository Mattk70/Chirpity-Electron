...
class PreprocessSpectrogramLayer extends tf.layers.Layer {
    ...
    return tf.tidy(() => {
      ...
      if (this.version === "v4") {
        ...
        const normalized = tf.div(
          tf.sub(inputs, spec_min),
          tf.sub(spec_max, spec_min)
        );
        return normalized;
      } else {
  ...
  build(inputShape) {
    this.inputSpec = [
      { shape: [undefined, inputShape[1], inputShape[2], inputShape[3]] },
    ];
    return this;
  }
...
onmessage = async (e) => {
  ...
  if (message === "load") {
    ...
    let { audio, start, fileStart, file, snr, worker, threshold, confidence } =
      e.data.payload;
