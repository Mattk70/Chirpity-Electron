...
class Model {
    ...
    async predictChunk(chunk, index) {
        ...
        tf.tidy(() => {
            ...
            //prepare summary
            console.log(index / this.config.sampleRate, primary, this.labels[primary], score);
        })
...
onmessage = async function (e) {
    ...
    if (modelRequest === 'load') {
        ...
        await myModel.warmUp();
        postMessage({message: 'model-ready', sampleRate: myModel.config.sampleRate, chunkLength: myModel.chunkLength})
