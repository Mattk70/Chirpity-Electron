...
class Model {
    ...
    async predictChunk(chunks, fileStart) {
        ...
        tf.tidy(() => {
            ...
            this.prediction = this.model.predict(this.batch, {batchSize: 2})
            // Get label
            ...
            for (let j = 0; j < top3.length; j++) {
                batch.push({'index': top3[j], 'score': top3scores[j]});
            }
            ...
            batch.forEach(function (item, index) {
                for (let i = 0; i < item.index.length; i++) {
                    ...
                    result = ({
                        start: keys[index] / CONFIG.sampleRate,
                        end: (keys[index] / CONFIG.sampleRate) * CONFIG.specLength,
                        timestamp: myModel._timestampFromSeconds(keys[index] / CONFIG.sampleRate, fileStart),
                        position: myModel._timestampFromSeconds(keys[index] / CONFIG.sampleRate, 0),
                        sname: labels[item.index[0]].split('_')[0],
                        cname: labels[item.index[0]].split('_')[1],
                        score: item.score[0],
                        sname2: labels[item.index[1]].split('_')[0],
                        cname2: labels[item.index[1]].split('_')[1],
                        score2: item.score[1],
                        sname3: labels[item.index[2]].split('_')[0],
                        cname3: labels[item.index[2]].split('_')[1],
                        score3: item.score[2],
                        suppressed: suppressed
                    });
                    audacity = ({
                        timestamp: (keys[index] / CONFIG.sampleRate) + '\t' + (keys[index] / CONFIG.sampleRate) +  CONFIG.specLength,
                        cname: labels[item.index[0]].split('_')[1],
                        score: item.score[0]
                    })
                    //prepare summary
                    console.log(index / CONFIG.sampleRate, item.index[0], labels[item.index[0]], item.score[0]);
...
onmessage = async function (e) {
    ...
    if (modelRequest === 'load') {
        ...
        const batched = await myModel.predictChunk(chunks, fileStart);
