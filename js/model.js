...
class Model {
    ...
    makeSpectrogram(audioBuffer) {
        ...
        */
        ...
        spectrogram = this._normalize(spectrogram);
        // Add channel axis
    ...
    predictBatch(file, fileStart) {
        ...
        this.batch = tf.image.resizeBilinear(this.batch, [height, width]);
...
async function runPredictions(e) {
    ...
    if (modelRequest === 'load') {
        ...
        tf.tidy(() => {
            ...
            image = tf.image.resizeBilinear(image, [height, width]).dataSync();
        })
