...
class Model {
    ...
    setList() {
        ...
        if (this.list === 'birds') {
            ...
            birdlist.forEach(species => allowed_IDs.push(set_labels.indexOf(species)))
            ...
            migrantlist.forEach(species => allowed_IDs.push(labels.indexOf(species)))
            // generate the opposite for a blacklist
            let i = 0;
            while (i < labels.length) {
                if (allowed_IDs.indexOf(i) === -1) {
    ...
    predictChunk(chunks, fileStart, file, duration) {
        let batched_results = [];
        let result;
        let audacity;
        let tensorArray = [];
