...
class Model {
    ...
    fixUpSpecBatch(specBatch) {
        ...
        return tf.image.resizeBilinear(specBatch, [height, width]);
    }
    ...
    predictBatch(file, fileStart) {
        ...
        //const padding  = tf.zeros([this.batchSize / 3, prediction.shape[1], 3]);
        //const paddedPrediction =  tf.concat([prediction, padding], -1);
        // let predictionAverages = prediction.reshape([this.batchSize / 3, prediction.shape[1],  3] );
        // predictionAverages = predictionAverages.mean(-1);
    ...
    predictChunk(chunks, fileStart, file, finalchunk, threshold) {
        return tf.tidy(() => {
...
async function runPredictions(e) {
    ...
    if (modelRequest === 'load') {
        ...
        const readyToSend = myModel.predictChunk(chunks, fileStart, file, finalChunk, SNRThreshold);
        if (readyToSend) {
            const response = {
                message: 'prediction',
                file: file,
        ...
        tf.tidy(() => {
            ...
            image = myModel.fixUpSpecBatch(image).dataSync();
        })
