...
const Toasts = { // UI.js
    ...
    es: {
        ...
        noSnameFound: "No se puede encontrar '${sname}' (en la línea ${line} de la lista personalizada) en la lista <strong>${model}</strong>. <strong>Consejos:</strong> <ol><li>¿Está su lista formateada como: <code>'Nombre científico_Nombre común'</code>?</li><li>¿Está su lista para el modelo <strong>${model}</strong>? Si no es así, cambie el modelo en la configuración</li><li>Mire si hay algún error ortográfico en el nombre de la especie</li></ol>",
    ...
    fr: {
        ...
        noSnameFound: "Impossible de trouver '${sname}' (à la ligne ${line} de la liste personnalisée) dans la liste <strong>${model}</strong>. <strong>Conseils :</strong> <ol><li>Votre liste est-elle formatée comme : <code>'Nom scientifique_Nom commun'</code> ?</li><li>Votre liste est-elle pour le modèle <strong>${model}</strong> ? Si ce n'est pas le cas, changez le modèle dans les paramètres</li><li>Vérifiez s'il y a une faute de frappe dans le nom de votre espèce</li></ol>",
    ...
    ja: {
        ...
        noSnameFound: "カスタムリストの${line}行目にある'${sname}'が<strong>${model}</strong>モデルリストに見つかりません。<strong>ヒント:</strong> <ol><li>リストの形式は次のようになっていますか：<code>'学名_一般名'</code></li><li>リストは<strong>${model}</strong>モデル用ですか？そうでない場合は、設定でモデルを変更してください</li><li>種名にタイプミスがないか確認してください</li></ol>",
    ...
    nl: {
        ...
        noSnameFound: "Kan '${sname}' niet vinden (op regel ${line} van de aangepaste lijst) in de <strong>${model}</strong> lijst. <strong>Tips:</strong> <ol><li>Is je lijst opgemaakt als: <code>'Wetenschappelijke naam_Gewone naam'</code>?</li><li>Is je lijst voor het <strong>${model}</strong> model? Zo niet, wijzig het model in de instellingen</li><li>Controleer op typfouten in de naam van je soort</li></ol>",
    ...
    sv: {
        ...
        noSnameFound: "Kunde inte hitta '${sname}' (på rad ${line} i den anpassade listan) i <strong>${model}</strong> listan. <strong>Tips:</strong> <ol><li>Är din lista formaterad som: <code>'Vetenskapligt namn_Allmänt namn'</code>?</li><li>Är din lista för modellen <strong>${model}</strong>? Om inte, ändra modellen i inställningarna</li><li>Kontrollera om det finns ett stavfel i artnamnet</li></ol>",
    ...
    zh: {
        ...
        noSnameFound: "无法在 <strong>${model}</strong> 列表中找到 '${sname}'（位于自定义列表的第 ${line} 行）。<strong>提示：</strong><ol><li>您的列表格式是否如下：<code>'学名_常用名'</code>？</li><li>您的列表是针对 <strong>${model}</strong> 模型的吗？如果不是，请在设置中更改模型。</li><li>检查物种名称是否有拼写错误。</li></ol>",
