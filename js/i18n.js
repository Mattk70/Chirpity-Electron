const i18nToasts = { // UI.js
    ...
    es: {
        ...
        analysisUnderway: "Un análisis está en curso. Presione <b>Esc</b> para cancelarlo antes de iniciar un nuevo análisis.",
        ...
        changeListBlocked: "No es posible cambiar la configuración de la lista mientras un análisis está en curso. Sin embargo, la lista <b>sÍ</b> se puede cambiar después de que termine el análisis.",
        ...
        noDLL: 'Ha ocurrido un error al cargar el modelo. Esto puede deberse a la falta de soporte para AVX. Los modelos AI de Chirpity requieren el conjunto de instrucciones AVX2 para funcionar. Si tiene AVX2 habilitado y sigue viendo este mensaje, consulte <a href="https://github.com/Mattk70/Chirpity-Electron/issues/84" target="_blank">este problema</a> en Github.',
        noFile: "No se puede localizar: ${file}",
        ffmpeg: 'Error de FFMPEG al extraer audio: ${error}',
        noNight: 'No se encontraron detecciones. ${file} no tiene un periodo en el que se puedan hacer predicciones. <b>Consejo:</b> Para ver detecciones en este archivo, desactive el modo nocmig y ejecute nuevamente el análisis.',
        ...
        noSnameFound: "No se puede encontrar '${sname}' (en la línea ${line} de la lista personalizada) en la lista <strong>${model}</strong>. <strong>Consejos:</strong> <ol><li>¿Está su lista para el modelo <strong>${model}</strong>? Si no es así, cambie el modelo en la configuración</li><li>Verifique si hay un error tipográfico en el nombre de la especie</li></ol>",
        noArchive: "No se puede acceder a la ubicación del archivo: ${location}. <br> Operación abortada",
    ...
    fr: {
        ...
        noCallCache: "Aucun cache d'appels trouvé.",
        callCacheCleared: "Le cache d'appels a été vidé avec succès.",
    ...
    ru: {
        ...
        maxFiles: "Chirpity ограничивает максимальное количество открытых файлов до 25 000. Будут открыты только первые 25 000 из ${STATE.openFiles.length} попыток.",
        analysisUnderway: "Анализ в процессе. Нажмите <b>Esc</b>, чтобы отменить его, прежде чем запустить новый анализ.",
        placeOutOfBounds: "Широта должна быть от -90 до 90, а долгота от -180 до 180.",
        placeNotFound: "Не удалось найти это местоположение. Пожалуйста, проверьте ваше интернет-соединение или попробуйте снова позже.",
        mustFilterSpecies: "Отфильтруйте результаты по видам для экспорта аудиофайлов.",
        noNode: "Стандартный бэкенд не удалось загрузить на этом устройстве. Вместо него используется экспериментальный бэкенд (webGPU).",
        badMessage: "Неизвестное сообщение от worker: ${args.event}",
        changeListBlocked: "Невозможно изменить настройки списка, пока идет анализ. Однако список <b>можно</b> изменить после завершения анализа.",
        cancelled: "Операция отменена",
        badTime: "Неверный формат времени. Пожалуйста, введите время в одном из следующих форматов: \n1. Число с плавающей запятой (в секундах) \n2. Два числа, разделенные двоеточием (для минут и секунд) \n3. Три числа, разделенные двоеточием (для часов, минут и секунд)",
        complete: "Анализ завершен.",
        feedback: "Спасибо, ваш отзыв помогает улучшить прогнозы Chirpity.",
        contextBlocked: "Невозможно изменить настройки контекста, пока идет анализ.",
        noCallCache: "Не найден кеш вызовов.",
        callCacheCleared: "Кеш вызовов успешно очищен.",
        badThreshold: "Пороговое значение должно быть числом от 0.001 до 1.",
        labelFileNeeded: "Вы должны выбрать файл меток в настройках списка, чтобы использовать опцию пользовательского языка.",
        listFileNeeded: "Необходимо загрузить пользовательский список для модели перед использованием опции пользовательского списка.",
        ...
        noLoad: 'Модель ${model} не загружена. Перезагрузите Chirpity для продолжения. Если вы видите это сообщение повторно, скорее всего, ваш компьютер не поддерживает AVX2, и Chirpity не будет работать на вашей системе.',
        noDLL: 'Произошла ошибка при загрузке модели. Это может быть связано с отсутствием поддержки AVX. Модели AI Chirpity требуют набора инструкций AVX2 для работы. Если у вас включен AVX2 и вы все еще видите это уведомление, обратитесь к <a href="https://github.com/Mattk70/Chirpity-Electron/issues/84" target="_blank">этому вопросу</a> на Github.',
        noFile: "Не удается найти: ${file}",
        ffmpeg: 'Ошибка FFMPEG при извлечении аудио: ${error}',
        noNight: 'Нет детекций. В ${file} нет периода, в котором могут быть даны прогнозы. <b>Совет:</b> Чтобы увидеть детекции в этом файле, отключите режим nocmig и выполните анализ снова.',
        saveBlocked: "Не удается сохранить файл ${filePath}\nтак как он открыт в другом приложении",
        goodSave: '${filePath} был успешно записан.',
        noDetections: "Не найдено детекций в выбранном разделе",
        noDetectionsDetailed: 'Не найдено детекций ${nocmig} ${species} в ${archive} с использованием списка ${list}.',
        noDetectionsDetailed2: 'Не найдено детекций в ${file}. Ищены записи с использованием списка ${list} с минимальной уверенностью ${confidence}%',
        dbNotLoaded: "База данных не завершила загрузку. Проверка наличия файла в архиве была пропущена",
        noSnameFound: "Не удалось найти '${sname}' (в строке ${line} пользовательского списка) в списке <strong>${model}</strong>. <strong>Советы:</strong> <ol><li>Ваш список для модели <strong>${model}</strong>? Если нет, измените модель в настройках</li><li>Проверьте на наличие опечатки в названии вида</li></ol>",
        ...
        allDaylight: "${file} не будет добавлен в архив, так как полностью находится в дневное время.",
        conversionDone: "Конвертация завершена для ${file}",
        badConversion: "Ошибка при конвертации файла ${file}: ${error}"
    },
    sv: {
...
const i18nTitles = {
    ...
    en: {
      ...
      "audioFiltersIcon": "Audio filters applied",
