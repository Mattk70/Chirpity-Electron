...
export class ChirpityWS  {
    constructor(container, getState, getConfig, handlers) {
 ...
 */
 ...
 init = ({ audio = undefined, height = 0 }) => {
    ...
    this.timeline = this.createTimeline(this.wavesurfer, windowLength, config);
    ...
    this.wavesurfer = WaveSurfer.create({
        ...
        cursorWidth: 2,
        height: "auto",
        renderFunction: this.nullRender, // no need to render a waveform
  ...
  wave.addEventListener("mousemove", this.specTooltip, { passive: true });
 ...
 */
 createTimeline(windowLength) {
  ...
  return ws ? ws.registerPlugin(timeline) : timeline;
}
...
async updateSpec({
  ...
  if (resetSpec || !wavesurfer) await this.adjustSpecDims(true);
  else {
